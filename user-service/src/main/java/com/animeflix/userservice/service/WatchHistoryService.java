...
public class WatchHistoryService {
    ...
    // Thêm/update watch history, đồng thời kích hoạt update continue-watching ở background (không chờ)
    public Mono<WatchHistoryResponse> addOrUpdateHistory(String userId, AddHistoryRequest request) {
        return historyRepo.findByUserIdAndAniIdAndEpId(userId, request.getAniId(), request.getEpId())
                ...
                .doOnNext(this::syncContinueWatchingAsync)
                .map(mapper::toResponse);
    }

    // Update continue-watching ở background — không block response trả về client
    private void syncContinueWatchingAsync(WatchHistory saved) {
    ...
    private Mono<WatchHistory> updateExisting(WatchHistory existing, AddHistoryRequest request) {
        ...
        if (request.getEpTitle() != null) existing.setEpTitle(request.getEpTitle());
        ...
        if (request.getDevice() != null) existing.setDevice(request.getDevice());
        if (request.getQuality() != null) existing.setQuality(request.getQuality());
    ...
    // Progress 0.0 - 1.0, an toàn với duration = null/0
    private Double calculateProgress(Double timeWatched, Double duration) {
        if (timeWatched == null || duration == null || duration == 0) return 0.0;
    ...
    public Flux<WatchHistoryResponse> getHistory(String userId, int page, int size) {
        ...
        return historyRepo.findByUserIdOrderByCreatedAtDesc(userId, pageable).map(mapper::toResponse);
    }

    // Lấy lịch sử xem của 1 anime cụ thể
    public Flux<WatchHistoryResponse> getHistoryByAnime(String userId, String aniId) {
        return historyRepo.findByUserIdAndAniIdOrderByCreatedAtDesc(userId, aniId).map(mapper::toResponse);
    }
