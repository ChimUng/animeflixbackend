...
public class AnifyClient {
    ...
    public AnifyClient(WebClient anifyWebClient, MalSyncClient malSyncClient) {
        this.webClient = anifyWebClient;
        this.malSyncClient = malSyncClient;
    }

    public Mono<StreamResponse> fetchAnifyEpisode(String provider, String episodeId, int episodeNum, String id, String subtype) {
        String uri = "/sources?providerId=" + provider + "&watchId=" + episodeId + "&episode=" + episodeNum + "&id=" + id + "&subType=" + subtype;
        return webClient.get()
                ...
                .onErrorMap(e -> new StreamFetchException("Anify fetch failed for episode ID: " + episodeId, e));
    }

    private StreamResponse parseStreamResponse(JsonNode response, Map<String, Object> ids) {
        StreamResponse streamResponse = new StreamResponse();
        streamResponse.setSuccess(true); // Giả định thành công
    ...
    private List<Subtitle> parseSubtitles(JsonNode subtitlesNode) {
        ...
        if (subtitlesNode.isArray()) {
            for (JsonNode subtitleNode : subtitlesNode) {
                ...
                subtitle.setLang(subtitleNode.path("language").asText("")); // Có thể đổi thành "lang" nếu API dùng thế
